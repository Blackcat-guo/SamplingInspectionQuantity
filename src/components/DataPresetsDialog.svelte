  ...
  <div class="dialog-list">
    {#if tab === 'customer'}
      ...
      {#each app.dataPresets.customers as c (c.id)}
        <div class="preset-edit-row">
          ...
          <div class="row-actions">
            <button class="scope-btn" onclick={() => toggleCustomerResp(c.id)}>
              负责人({c.responsibleIds.length})
            </button>
            <button class="scope-btn" onclick={() => toggleCustomerProducts(c.id)}>
              关联产品({countCustomerProducts(c.name)})
            </button>
          ...
          <div class="resp-panel">
            ...
            {#each normalResponsibles() as r (r.id)}
              <label class="responsible-item">
                <input
                  type="checkbox"
                  checked={c.responsibleIds.includes(r.id)}
        ...
        {#if uiState.expandedCustomerProductsId === c.id}
          <div class="resp-panel">
            ...
            <div class="panel-toolbar">
              <input
                value={uiState.productPickerFilter}
      ...
      <div class="preset-add-row">
        <input
          ...
          maxlength="40"
          onkeydown={(e) => {
            if (e.key === 'Enter' && addDataPreset('customer', newCustomer)) newCustomer = '';
          }}
        />
        <button
          onclick={() => {
            if (addDataPreset('customer', newCustomer)) newCustomer = '';
          }}
        >添加</button>
      </div>
    {/if}
    ...
    {#if tab === 'supplier'}
      ...
      {#each app.dataPresets.suppliers as s, i (s)}
        <div class="preset-edit-row">
          ...
          />
          <div class="row-actions">
            <button class="scope-btn" onclick={() => toggleSupplierProducts(s)}>
              关联产品({countSupplierProducts(s)})
            </button>
      ...
      <div class="preset-add-row">
        <input
          ...
          maxlength="40"
          onkeydown={(e) => {
            if (e.key === 'Enter' && addDataPreset('supplier', newSupplier)) newSupplier = '';
          }}
        />
        <button
          onclick={() => {
            if (addDataPreset('supplier', newSupplier)) newSupplier = '';
          }}
        >添加</button>
      </div>
    {/if}
    ...
    {#if tab === 'incoming'}
      ...
      <div class="preset-add-row">
        ...
        <button
          onclick={() => {
            if (addDataPreset('incoming', newIncoming)) newIncoming = '';
          }}
        >添加</button>
      </div>
    {/if}
    ...
    {#if tab === 'process'}
      ...
      <div class="preset-add-row">
        ...
        <button
          onclick={() => {
            if (addDataPreset('process', newProcess)) newProcess = '';
          }}
        >添加</button>
      </div>
    {/if}
