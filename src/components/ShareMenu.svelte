<script lang="ts">
  ...
  function getText(): string {
    return buildMergedText(false);
  }

  async function doSystem() {
    if (!shareable) {
      pushToast('当前浏览器不支持系统分享', 'error', 2200);
      return;
    }
    const text = getText();
    open = false;
...
>
  <div class="dialog-list">
    <button
      type="button"
      class="share-option"
      class:disabled={!shareable}
      disabled={!shareable || !canShare}
    ...
    >
      ...
      <span class="so-label">
        系统分享（推荐）
        <div class="so-hint">
          {shareable ? '调起系统分享面板，可直接选目标 App' : '当前浏览器不支持 Web Share API'}
        </div>
      </span>
    </button>
