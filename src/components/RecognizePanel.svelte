<script lang="ts">
  ...
  onMount(() => { initRecognizeTools(); });

  $effect(() => {
    return () => { clearOcrFiles(); };
  });
  ...
  const selectedCount = $derived(recognizeState.candidates.filter((c) => c.checked).length);
  ...
  function onPickFiles() { fileInputEl?.click(); }
...
<section class="box">
  ...
  <div class="info-field" style="margin-bottom:8px">
    ...
    <div style="display:flex;gap:6px">
      <input type="text" value={recognizeState.ocrApiKey}
             placeholder="留空则使用 helloworld（次数受限）"
             oninput={(e) => (recognizeState.ocrApiKey = (e.target as HTMLInputElement).value)}
             style="flex:1" />
      <button class="mini-batch-btn" onclick={() => saveOcrApiKey(recognizeState.ocrApiKey)}>保存</button>
    </div>
  </div>
  <p class="backup-note">⚠️ Key 会以请求头发送到 ocr.space；仅存 sessionStorage，关闭标签页即清除。</p>
</section>
...
<section class="box">
  ...
  <input bind:this={fileInputEl} type="file" accept="image/*" multiple onchange={onFileChange} style="display:none" />
  <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px">
    <button class="mini-batch-btn" style="flex:1" onclick={onPickFiles}>🖼️ 从相册选择图片</button>
    <button class="mini-batch-btn danger"
            disabled={!recognizeState.ocrFiles.length || recognizeState.ocrRunning}
            onclick={clearOcrFiles}>清空</button>
  </div>
    ...
    <div class="recognize-preview-grid">
      {#each recognizeState.ocrPreviews as url, i (url)}
        <div class="recognize-preview-item">
          ...
          <button class="recognize-preview-del" aria-label="删除第 {i + 1} 张"
                  disabled={recognizeState.ocrRunning} onclick={() => removeOcrFile(i)}>✕</button>
        </div>
      {/each}
    </div>
  {/if}
  {#if recognizeState.ocrRunning || recognizeState.ocrProgress > 0}
    <div class="recognize-progress">
      <div class="recognize-progress-inner" style="width: {recognizeState.ocrProgress}%"></div>
    </div>
  {/if}
  {#if recognizeState.ocrStatus}
    <div class="recognize-status {recognizeState.ocrStatusType}">{recognizeState.ocrStatus}</div>
  {/if}
  <button class="mini-batch-btn" style="width:100%;padding:10px"
          disabled={recognizeState.ocrRunning || !recognizeState.ocrFiles.length}
          onclick={startOcrRecognition}>
    {recognizeState.ocrRunning ? '识别中…' : '开始识别'}
  </button>
</section>
...
<section class="box">
  ...
  <div class="info-field" style="margin-bottom:8px">
    ...
    <select aria-label="拆分方式" value={recognizeState.splitMode}
            onchange={(e) => (recognizeState.splitMode = (e.target as HTMLSelectElement).value as any)}>
  ...
  <textarea bind:value={recognizeState.text}
            placeholder="粘贴 OCR / 语音 / 手工文本…"
            style="width:100%;min-height:100px;padding:10px;font-size:13.5px;line-height:1.7;border:1.5px solid var(--c-border);border-radius:10px;background:var(--c-surface);color:var(--c-text);outline:none;resize:vertical;font-family:inherit;margin-bottom:8px"></textarea>
  <div style="display:flex;gap:8px;flex-wrap:wrap">
    <button class="mini-batch-btn" style="flex:1" onclick={requestGenerateCandidates}>🔍 生成候选列表</button>
    <button class="mini-batch-btn" disabled={!recognizeState.text.trim()} onclick={applyAllRecognizedText}>全部应用</button>
    <button class="mini-batch-btn danger"
            disabled={!recognizeState.text && !recognizeState.candidates.length}
            onclick={clearRecognizeText}>清空</button>
  </div>
</section>
...
{#if recognizeState.candidates.length}
  <section class="box">
    {#if recognizeState.candidatesSourcePid && recognizeState.candidatesSourcePid !== currentProduct()?.id}
      <div class="recognize-status warn">⚠️ 候选生成于其他产品，请重新生成</div>
    {/if}
      ...
      {#each recognizeState.candidates as c (c.id)}
        <div class="recognize-candidate-row" class:matched={c.matched}>
          <label style="display:inline-flex;align-items:center;gap:8px;flex:1;min-width:0">
            <input type="checkbox" checked={c.checked} onchange={() => toggleCandidate(c.id)} />
            <span style="flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{c.text}</span>
          </label>
          {#if c.matched}
          ...
          {:else}
            <select aria-label="目标分组" value={c.groupId}
    ...
    <button class="mini-batch-btn"
            ...
            disabled={!selectedCount} onclick={applySelectedCandidates}>
