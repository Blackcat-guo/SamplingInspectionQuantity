<script lang="ts">
  ...
  async function requestMic() {
    if (!envMic) { pushToast('当前浏览器不支持麦克风 API', 'error'); return; }
    try {
  ...
  async function copyUrl() {
    try { await navigator.clipboard.writeText(location.href); pushToast('已复制当前地址'); }
    catch { pushToast('复制失败', 'error'); }
  }

  const hasText = $derived(!!(recognizeState.voiceFinalText || recognizeState.voiceInterimText));
</script>
...
{#if canRender}
  <section class="box">
    ...
    <div class="voice-env-card">
      <div class="voice-env-row"><span>安全上下文（HTTPS）</span><span class="voice-env-val">{envSecure ? '✓' : '✗'}</span></div>
      <div class="voice-env-row"><span>麦克风 API</span><span class="voice-env-val">{envMic ? '✓' : '✗'}</span></div>
      <div class="voice-env-row"><span>语音识别 API</span><span class="voice-env-val">{envSR ? '✓' : '✗'}</span></div>
    ...
    <div class="voice-actions">
      {#if !recognizeState.voiceRunning}
        <button class="mini-batch-btn" style="flex:1;padding:10px" onclick={startVoiceRecognition}>🎤 开始语音</button>
      {:else}
        <button class="mini-batch-btn danger" style="flex:1;padding:10px" onclick={stopVoiceRecognition}>⏹ 停止语音</button>
      {/if}
      {#if hasText}
    ...
    {#if recognizeState.voiceStatus}
      <div class="recognize-status {recognizeState.voiceStatusType}">{recognizeState.voiceStatus}</div>
    {/if}

    <div class="voice-apply-row">
      <button class="mini-batch-btn" disabled={!hasText} onclick={applyVoiceToRecognizeText}>填入识别文本</button>
      <button class="mini-batch-btn" disabled={!hasText} onclick={applyVoiceToCandidates}>生成候选列表</button>
    </div>
  </section>
{/if}
