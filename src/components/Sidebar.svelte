<script lang="ts">
  ...
  const filteredList = $derived.by(() => {
    ...
    const list = !q
      ? app.products
      : app.products.filter((p) => p.name.toLowerCase().includes(q));
  ...
  {#if batchMode}
    <div style="margin:8px 8px 0;padding:8px 10px;background:var(--c-primary-soft);border:1.5px solid var(--c-primary-border);border-radius:10px;font-size:12px;color:var(--c-primary-dark)">
      ...
      <div style="display:flex;gap:6px;margin-top:7px">
        ...
        >🗑 删除所选</button>
        <button
          onclick={() => { batchMode = false; selected = []; }}
    ...
    {#each filteredList as p (p.id)}
      ...
      >
        ...
        {#if !batchMode}
          <button
            class="product-edit"
            onclick={(e) => { e.stopPropagation(); renameProduct(p.id); }}
            title="重命名"
          >✏️</button>
          <button
            class="product-copy"
            onclick={(e) => { e.stopPropagation(); duplicateProduct(p.id); }}
            title="复制该产品"
          >⧉</button>
          <button
            class="product-del"
