<script lang="ts">
  ...
  function toggleSort() { sortDir = sortDir === 'asc' ? 'desc' : sortDir === 'desc' ? '' : 'asc'; }
  function toggleBatch() { batchMode = !batchMode; selected = []; }
  function toggleAll(checked: boolean) { selected = checked ? filteredList.map((p) => p.id) : []; }
...
<aside class="sidebar" class:open={app.sidebarOpen} aria-label="产品列表">
  ...
  <div class="product-list">
    {#each filteredList as p (p.id)}
      <div class="product-item" class:active={p.id === app.currentProductId && !batchMode}
           onclick={() => onItemClick(p.id)} role="button" tabindex="0"
           onkeydown={(e) => { if (e.key === 'Enter') onItemClick(p.id); }}>
        {#if batchMode}
          <input type="checkbox" class="batch-check" checked={selected.includes(p.id)}
                 onclick={(e) => e.stopPropagation()}
                 onchange={() => toggleOne(p.id)} aria-label={'选择产品 ' + p.name} />
        {/if}
