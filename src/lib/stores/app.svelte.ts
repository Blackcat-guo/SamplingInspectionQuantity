...
<ol>
  <li>左上角「＋ 添加产品」录入料号</li>
  <li>「添加分组」→ 输入 5 或 3-7 批量创建</li>
  <li>在分组内添加分类并录入数量</li>
</ol>
<blockquote>📘 <b>案例：料号 5004 完整录入</b><br>
① 添加产品输入 <code>5004</code><br>
② 添加分组输入 <code>3</code> → 得到「分组1/2/3」<br>
③ 在分组1输入 <code>磕碰伤</code> 回车<br>
④ 数量 +1 至 5，总数量填 20<br>
⑤ 下方「本产品汇总」自动生成：
<pre>客户：（空）
供应商来料：A厂
...
5004-2
5004-3</pre>
或写成一行：<code>5004-1, 5004-2, 5004-3</code><br>
...
<h4>3.1 添加分组</h4>
<ul>
  <li>输入单数字 <code>5</code> → 创建「分组1~分组5」</li>
  <li>输入区间 <code>3-7</code> → 创建「分组3~分组7」</li>
  <li>输入逗号 <code>1,3,5</code> → 创建「分组1、分组3、分组5」</li>
  <li>输入文字 <code>外观</code> → 创建名为「外观」的分组</li>
</ul>
<h4>3.2 一键标准分组</h4>
<p>点「📋 标准分组」→ 一键添加 严重 / 主要 / 次要 三组。</p>
...
③ 点「常见外观问题」→ 主面板出现一个新分组，包含 4 个分类<br>
...
<h4>7.1 两种导出</h4>
<ul>
  <li><b>全部导出</b>：一键打包所有数据（含产品 + 预设 + 设置）</li>
  <li><b>部分导出</b>：弹窗中勾选需要的模块</li>
</ul>
<h4>7.2 支持的文件格式</h4>
<ul>
...
① 设置 → 备份与恢复 → 「💾 导出全部数据」<br>
② 下载 <code>抽检数量统计_20260921.json</code> 保存到网盘<br>
③ 换设备后 → 「📥 导入全部数据（合并）」<br>
④ 选择文件 → 二次确认「合并导入？现有数据保留，重复项跳过」<br>
⑤ 导入完成，Toast 提示「导入完成」
</blockquote>
<h4>7.3 部分导入</h4>
...
① 误删了「主要」分组 → 立刻按 <kbd>Ctrl</kbd>+<kbd>Z</kbd><br>
② Toast 显示「已撤回：删除分组「主要」」<br>
③ 分组恢复原位<br>
